...
# ---------------- JWT DECODE → account_id ---------------- #
def decode_jwt_token(jwt_token):
    """Extract account_id from JWT payload (TargetCodex_V7 method)"""
    try:
...
def generate_random_name(name_prefix):
    return f"{name_prefix[:7]}{generate_exponent_number()}"


def generate_custom_password():
    characters = string.ascii_letters + string.digits
    random_part = ''.join(random.choice(characters) for _ in range(7)).upper()
    return f"TargetCodex_{random_part}"
    ...
    if RESPONSE.status_code == 200:
        ...
        try:
            ...
            if start_idx != -1:
                ...
                # Extract account_id from JWT (TargetCodex_V7 method)
                account_id = decode_jwt_token(BASE64_TOKEN)
                dat = GET_PAYLOAD_BY_DATA(BASE64_TOKEN, access_token, 1, response_hex, status_code, name, uid, password, region, account_id)
                return dat
        except Exception:
            pass
...
def login_server(uid, password, access_token, open_id, response, status_code, name, region):
    ...
    if RESPONSE.status_code == 200:
        ...
        if BASE64_TOKEN:
            ...
            # Extract account_id from JWT (TargetCodex_V7 method)
            account_id = decode_jwt_token(BASE64_TOKEN)
            dat = GET_PAYLOAD_BY_DATA(BASE64_TOKEN, access_token, 1, response, status_code, name, uid, password, region, account_id)
            return dat
...
def GET_PAYLOAD_BY_DATA(JWT_TOKEN, NEW_ACCESS_TOKEN, date, response, status_code, name, uid, password, region, account_id="N/A"):
    try:
